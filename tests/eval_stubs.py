...
def citation(**overrides) -> Citation:
    fields = dict(
        id="c1", chunk_id="user_manual-p11-1", document="user_manual.pdf", page=11,
        page_in_language=None, section="4. START-UP INSTRUCTIONS", quote="PRIOR TO START-UP",
    )
    return Citation(**(fields | overrides))


def response(outcome: str = "answer", message: str | None = None, citations: list[Citation] | None = None) -> AskResponse:
    if outcome == "answer":
        message = "Check before start-up [c1]." if message is None else message
        citations = [citation()] if citations is None else citations
    return AskResponse(
        outcome=outcome, language="en", message=message or f"{outcome} message",
        citations=citations or [], warnings=[],
