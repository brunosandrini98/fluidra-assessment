...
- "answer": the message in the user's language ({language}). End every claim with a marker [chunk_id] naming the chunk that supports it. Give exactly one citation per cited chunk, with a verbatim quote of at most 200 characters copied from that chunk. Include the safety instructions (warnings, precautions) that the retrieved chunks give for the task. When the answer depends on a condition, give each branch with its citation. Use square brackets only for markers; write figure references as "Fig. 5".
